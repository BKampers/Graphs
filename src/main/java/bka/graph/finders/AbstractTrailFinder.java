...
public abstract class AbstractTrailFinder<V, E extends Edge<V>> implements TrailFinder<V, E> {
    ...
    protected Collection<E> complement(Collection<E> edges, V vertex) {
        return edges.stream().filter(Predicate.not(EdgeUtil.isIncidentWith(vertex)).and(Predicate.not(EdgeUtil::isLoop))).collect(Collectors.toList());
    }

    protected static <T> boolean pass(Predicate<T> predicate, T t) {
